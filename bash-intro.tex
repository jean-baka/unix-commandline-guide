...
\begin{itemize}
	\item \gc{rm file1}: to \textbf{r}e\tb{m}ove \verb+file1+ (must be present in current dir)
	...
	\item \gc{touch newfile} to create (\textbf{touch}, as it changes the date of last modification) an empty file in the current directory
...
\begin{itemize}
	...
	\item \gc{\string[\string]} to provide a list of characters to pick from: \gc{ls file[189]} will pick \texttt{file1}, \texttt{file8} and \texttt{file9} only
...
\emph{Weak quoting} with double quotes will not prevent variable interpretation:
	\gc{a=5; echo "\$a"} prints ``\texttt{5}''\\
	Quotes are essential to include spaces in your text:\\
	\gc{myvar="hello boy!"}\\
\vspace{3ex}
\emph{Strong quoting} prevents interpretation of basically everything:
