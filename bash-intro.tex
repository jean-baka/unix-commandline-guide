...
\begin{frame}[fragile]
	...
	\begin{itemize}
		...
		\item \gc{[ "\$a" < "\$b" ]} true if string \$a is (in ASCII alphabetical order) before string \$b
		\item \gc{[ "\$a" <= "\$b" ]} true if string \$a is (in ASCII alphabetical order) before string \$b, or equal to it
	\end{itemize}
\end{frame}
